...
Public Class FormMenuRelatorioCaixa
    ...
    Dim ListOperacoesRelatorioCaixa As New List(Of ClCaixa)
    ...
    Sub executaF11()
        ...
        Formulario.TotalLiquido = TotalLiquido
    ...
    Sub PreencheListaVendasFinalizadas(caixa As ClAbrirFecharCaixa)
        ...
        If MdlConexaoBD.sincronizado Then
            _ListVendaServicos = MdlConexaoBD.ListaVendaServico.FindAll(Function(vs As ClVendaServico) vs.vsdatahora >= caixa.Af_datahora1)
        Else
            _VendaServicoDAO.TrazListVendaServicoBD(_ListVendaServicos, MdlConexaoBD.conectionPadrao, Where)
        End If
    ...
    Sub PreencheListaAReceberPagas(caixa As ClAbrirFecharCaixa)
        ...
        Catch ex As Exception
            MsgBox("ERRO ao trazer Contas ARECEBER :: " & ex.Message, MsgBoxStyle.Exclamation, Application.CompanyName.ToUpper)
            Return
    ...
    Sub PreencheListaOperacoesBD(caixa As ClAbrirFecharCaixa)
        ...
        Try
            ...
            While dr.Read
                ...
                With RelatorioAux
                    ...
                    If dr(0).ToString.Equals("T") Then .coluna1 += "TROCO"
    ...
    Sub PreencheListaOperacoes_Sincronizado(caixa As ClAbrirFecharCaixa)

        ListOperacoesRelatorio.Clear()
        ListOperacoesRelatorioCaixa = MdlConexaoBD.ListaCaixas.FindAll(Function(cx As ClCaixa) cx.Cx_af_id1 = caixa.Af_id1)
        ListOperacoesRelatorioCaixa.Sort(Function(cx1 As ClCaixa, cx2 As ClCaixa) cx1.Cx_datahora1 = cx2.Cx_datahora1)
        If ListOperacoesRelatorioCaixa.Count < 1 Then Return


        RelatorioAux.zeraValores()
        If (ListOperacoesRelatorio.Count < 1) AndAlso ListOperacoesRelatorioCaixa.Count > 0 Then
            RelatorioAux.coluna1 = "Movimentaçoes do Caixa:: "
            ListOperacoesRelatorio.Add(New ClRelatorioPadraoSTR(RelatorioAux))
        End If

        Dim mTelefone As String = ""
        For Each cx As ClCaixa In ListOperacoesRelatorioCaixa

            RelatorioAux.zeraValores()

            With RelatorioAux

                .coluna1 = cx.Cx_tipo1 & " - " & cx.Cx_motivo1
                If cx.Cx_tipo1.Equals("T") Then .coluna1 += "TROCO"
                .coluna2 = cx.Cx_datahora1.ToShortDateString & " " & cx.Cx_datahora1.ToShortTimeString
                .coluna3 = cx.Cx_valor1.ToString("#,##0.00")
                TotGeralOperacoes += cx.Cx_valor1
                .coluna4 = cx.Cx_tipo_valor1

            End With
            ListOperacoesRelatorio.Add(New ClRelatorioPadraoSTR(RelatorioAux))

            'Soma os totais:
            Select Case cx.Cx_tipo1
                Case "E" 'Entradas
                    TotalEntradas += cx.Cx_valor1
                    Select Case cx.Cx_tipo_valor1
                        Case "DN"
                            TotalLiquido += cx.Cx_tipo_valor1
                            'Case "CTC"
                            '    TotalCTC += cx.Cx_valor1
                            'Case "CTD"
                            '    TotalCTD += cx.Cx_valor1
                            'Case "PIX"
                            '    TotalPIX += cx.Cx_valor1
                            'Case "TRA"
                            '    TotalTRA += cx.Cx_valor1
                            'Case "CRD"
                            '    TotalCRD += cx.Cx_valor1
                    End Select

                Case "S" 'Saidas
                    TotalSaidas += cx.Cx_valor1
                    Select Case cx.Cx_tipo_valor1
                        Case "DN"
                            TotalLiquido -= cx.Cx_tipo_valor1
                            'Case "CTC"
    ...
    Sub configuraRelatorioBusca()
        ...
        TotalEntradas = 0 : TotalSaidas = 0 : TotalTroco = 0 : TotalRecebimento = 0 : TotalLiquido = 0
        ...
        If ListCaixaMarcados.Count > 0 Then
            ...
            For Each l As ClAbrirFecharCaixa In ListCaixaMarcados
                ...
                If rdbTodos.Checked Then
                    ...
                    If MdlConexaoBD.sincronizado Then
                        PreencheListaOperacoes_Sincronizado(l)
                    Else
                        PreencheListaOperacoesBD(l)
                    End If
        ...
        For Each r As DataGridViewRow In dtgCaixas.Rows
            ...
            If r.IsNewRow = False Then
                ...
                If r.Cells(1).Value Then
                    ...
                    If MdlConexaoBD.sincronizado Then
                        DaoCaixa.trazAbrirFechar_DaLista(CaixaRelatorio, MdlConexaoBD.ListaAbrirFecharCX)
                    Else
    ...
    Function validaData() As Boolean
        ...
        If CDate(dtpInicio.Text) > CDate(dtpFim.Text) Then
            MsgBox("Data Inicial dever Menor ou Igual a Data Final!", MsgBoxStyle.Exclamation, Application.CompanyName.ToUpper)
            dtpInicio.Focus()
            Return False
        End If
    ...
    Sub TrazCaixas()
        ...
        If MdlConexaoBD.sincronizado Then
            lAbrirFecharTodos = mAbrirFecharDAO.trazListaAbrirFechar_DaLista(MdlConexaoBD.ListaAbrirFecharCX)
            lMovCaixaTodos = mCaixaDAO.trazListaCaixa_DaLista(MdlConexaoBD.ListaCaixas)
        Else
    ...
    Sub BuscaCaixaPorPeriodo()
        ...
        If MdlConexaoBD.sincronizado Then
            lAbrirFecharTodos = mAbrirFecharDAO.trazListaAbrirFechar_DaLista(MdlConexaoBD.ListaAbrirFecharCX)
            lAbrirFecharTodos = lAbrirFecharTodos.FindAll(Function(cx As ClAbrirFecharCaixa) cx.Af_datahora1.ToShortDateString >= dtpInicio.Text And _
                                                              cx.Af_datahora1.ToShortDateString <= dtpFim.Text)
        Else
    ...
    Sub preenchDtgCaixas()
        ...
        If validaData() Then
            ...
            If lCaixasAbertos.Count <= 0 AndAlso lCaixasFechados.Count <= 0 Then
                If count > 1 Then MsgBox("Não Existe Caixas Nesse Período!") : Return
            End If
    ...
    Private Sub btnBuscaCaixas_Click(sender As Object, e As EventArgs) Handles btnBuscaCaixas.Click
        preenchDtgCaixas()
    End Sub
